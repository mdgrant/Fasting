...
study_char_cho.dat %>%
  ...
  kbl(
    ...
    col.names = c("\\ \\ \\ \\ \\ \\ \\ \\ ID", "Study", "Analyzed (N)", "Centers", "[Country^a^](#footnote_hdi)")) %>%
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>%
  kable_styling(bootstrap_options = opt_boot) %>%
...
```{r thirstMetaDichot, include = TRUE,  fig.width = 10, fig.height = 3, fig.align = "center"}
...
dichot_complicate %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "complicate" = 2, "OR (95% CI)"), line = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>% 
  kable_styling(bootstrap_options = opt_boot) %>% 
  column_spec(1, width = "9em") %>%  
  column_spec(2, width = "3em") %>% 
  ...
  pack_sub(., "RCT", 1, 60) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 61, 64) %>%
  pack_sub(., "Prospective Cohort", 65, 65) %>%
  pack_sub(., "Case-Control", 66, 67) %>%
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein.",
           alphabet = c("When outcome assessed relative to surgery where 0 is prior to induction."),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
```{r complicateMetaDichot, include = FALSE,  fig.width = 10, fig.height = 6.5, fig.align = "center"}
# <br>

### &nbsp;&nbsp; Pooled (adult RCTs) 

# <font size = 4> Figure `r figure_n`. Pooled incidence of complicateing --- randomized trials comparing carbohydrate drinks with fasting in adult surgical patients. </font>


## (updated 2021/01/13 09:48)  --------------------------------------------
# data
# cho vs fasting
complicate_meta.dat <- or_join(dichot_complicate_or, "complicate") %>% 
  filter(c_tx == "Fasting") %>% 
  filter(!refid %in% c(6530, 6529, 3367)) # observational

# Hausel
temp <- dichot_complicate_or %>% filter(refid == 5091)
a <- or_join(temp[c(1,3),], "complicate")
temp <- dichot_complicate_or %>% filter(refid == 2172)
b <- or_join(temp[c(1,3),], "complicate")

complicate_meta.dat <- bind_rows(complicate_meta.dat, a, b)
rm(a, b)
...
complicate_meta <- metabin(event_e, n_e, event_c, n_c,
  data = complicate_meta.dat,
  studlab = study,
  byvar = tx,
  sm = "OR",
  method = "Inverse",
...
```{r complicateMetaDichotPlacWater, include = FALSE,  fig.width = 10, fig.height = 4, fig.align = "center"}
# <br>
# <font size = 4> Figure `r figure_n`. Pooled incidence of complications --- randomized trials comparing carbohydrate drinks with water or placebo in adult surgical patients. </font>
...
# Hausel
temp <- dichot_complicate_or %>% filter(refid == 5091)
a <- or_join(temp[c(2,3),], "complicate")
# Singh
temp <- dichot_complicate_or %>% filter(refid == 2172)
b <- or_join(temp[c(2,3),], "complicate")
