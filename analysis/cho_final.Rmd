...
Indication of potential small-study effects (P = .06, Pustejovsky test of funnel plot asymmetry), but 3 studies pooled with large effects with uncertainty due to imputation. 
...
dichot_cho.dat %>%
  ...
  add_header_above(c(" " = 7, "CHO/Comp", "CHO/Simp", " " = 1), line = FALSE, bold = TRUE) %>% 
  ...
  row_spec(45, bold = TRUE) %>%
  column_spec(10, bold = TRUE) %>%
  pack_top(., "Adult", 1, 44) %>%
  pack_sub(., "RCT", 1, 37) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 38, 42) %>%
  pack_sub(., "Prospective Cohort", 43, 43) %>%
  pack_sub(., "Case-Control", 44, 44) %>%
  footnote(
