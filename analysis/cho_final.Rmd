...
```{r metaPh, include = TRUE, ECHO = FALSE, fig.width = 10, fig.height = 3, fig.align = "center"}
ph_meta.dat <- ph.dat %>% 
  filter(age == "Adult") %>% 
  arrange(design, year, study) %>%
  filter(arm_tx == "Fasting" | grepl("CHO", arm_tx)) %>%
...
```{r complicateMetaWaterPlac, include = TRUE, fig.width = 10, fig.height = 6, fig.align = "center"}
...
```{r complicateMetaFastProt, include = TRUE, fig.width = 10, fig.height = 2.5, fig.align = "center"}
## (updated 2021/05/11 10:14) cho vs fasting ------------------------------
temp <- metabin(
  event.e = comp_n_cho,
  n.e = arm_n_cho,
