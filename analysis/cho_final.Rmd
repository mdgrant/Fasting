...
```{r comfortAdultLikertMeta, echo = FALSE,  fig.width = 10, fig.height = 2.7, fig.align = "center"}
...
### Rates, Pediatric (preop) <a id="choVomitPeds25"></a>
...
ph.dat %>% 
  ...
  pack_sub(., "RCT", 16, 19) %>%
  result_pack("Pooled fixed-effects MD for 2 studies, 0.48 (95% CI, 0.31-0.64), *I^2^* = 98% ", 20, padding = 20) %>% 
  footnote(
    general = "RGV: residual gastric volumne; M: mean; SD: standard deviation; Med: median; IQR: interquartile range; CHO: carbohydrate; Simp: simple; Comp: complex; Prot: protein; RCT: randomized controlled trial.",
...
fasting_table %>%
  ...
  pack_top(., "Adult, Surgical", 1, 232-5) %>% 
  pack_sub(., "RCT", 1, 200-5) %>% 
  pack_sub(., "Nonrandomized Studies of Interventions", 201-5, 220-5) %>%
  pack_sub(., "Prospective Cohort", 221-5, 223-5) %>%
  pack_sub(., "Retrospective Cohort", 224-5, 228-5) %>% 
  pack_sub(., "Case-control", 229-5, 230-5) %>% 
  pack_sub(., "Before-after", 231-5, 232-5) %>% 
  pack_top(., "Adult, Non-surgical", 233-5, 260-5) %>% 
  pack_sub(., "RCT", 233-5, 234-5) %>%
  pack_sub(., "Crossover", 235-5, 255-5) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 256-5, 260-5) %>% 
  pack_top(., "Pediatric, Surgical", 261-5, 281-5) %>% 
  pack_sub(., "RCT", 261-5, 278-5) %>% 
  pack_sub(., "Nonrandomized Studies of Interventions", 279-5, 280-5) %>% 
  pack_sub(., "Prospective Cohort", 281-5, 281-5) %>% 
  pack_top(., "Pediatric, Non-surgical", 282-5, 287-5) %>% 
  pack_sub(., "RCT", 282-5, 284-5) %>%
  pack_sub(., "Crossover", 285-5, 287-5) %>%
  pack_sub(., "Prospective Cohort", 276-5, 276-5) %>% 
...
cho_drink_table %>% 
  ...
  pack_top(., "Adult, Surgical", 1, 121-2) %>% 
  pack_sub(., "RCT", 1, 101-2) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 102-2, 113-2) %>% 
  pack_sub(., "Prospective Cohort", 114-2, 116-2) %>%
  pack_sub(., "Retrospective Cohort", 117-2, 119-2) %>% 
  pack_sub(., "Case-control", 120-2, 120-2) %>% 
  pack_sub(., "Before-after", 121-2, 121) %>% 
  pack_top(., "Adult, Non-surgical", 122-2, 146-2) %>% 
  pack_sub(., "RCT", 122-2, 123-2) %>%
  pack_sub(., "Crossover", 124-2, 141-2) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 142-2, 146-2) %>% 
  pack_top(., "Pediatric, Surgical", 147-2, 159-2) %>% 
  pack_sub(., "RCT", 147-2, 157-2) %>% 
  pack_sub(., "Nonrandomized Studies of Interventions", 158-2, 158-2) %>% 
  pack_sub(., "Prospective Cohort", 159-2, 159-2) %>% 
  pack_top(., "Pediatric, Non-surgical", 160-2, 163-2) %>% 
  pack_sub(., "RCT", 160-2, 161-2) %>%
  pack_sub(., "Crossover", 162-2, 162-2) %>%
  pack_sub(., "Prospective Cohort", 163-2, 163-2) %>% 
