...
knitr::opts_chunk$set(echo = TRUE, options(knitr.kable.NA = '', dev = 'svg'))
library(kableExtra)
library(countrycode)
library(Cairo)
library(tidyverse)
...
<font size = 4> Table `r table_n`. Included studies. </font>
...
```{r, echo = FALSE}
#htmltools::tags$iframe(title = "My embedded document", src = "figures/gum_grade_112120.html", width = "1000", height = "420", frameBorder = "0")
```

### Strenght of Evidence Ratings

<font size = 4> Table `r table_n`. Strength of evidence ratings for outcomes and net benefit. </font>

```{r soeKable, echo = FALSE}
# read data
soe.dat <- readxl::read_xlsx("data/gum_soe_112120.xlsx", range = "A1:E8", sheet = "soe") %>% 
  janitor::clean_names()

soe.dat %>% 
  mutate(acc_aha = ifelse(outcome == "adults", str_c(acc_aha, "$^a$"), acc_aha)) %>% 
  kbl(booktabs = T, align = c("lcccc"),
      col.names = c(" ", "RCTs (n)", "GRADE", "ACC/AHA", "USPSTF")) %>%
  kable_classic(full_width = FALSE, html_font = "Source Sans Pro", position = "left", "hover") %>% 
  kable_styling(bootstrap_options = c("striped", "hover","condensed")) %>% 
  column_spec(1, width = "11em") %>% 
  column_spec(2, width = "6em") %>% 
  column_spec(c(3,4,5), width = "9em") %>% 
  pack_top(., "Outcome", 1, 3) %>% 
  pack_sub(., "Adults", 1, 3) %>% 
  pack_sub(., "Children", 4, 5) %>% 
  pack_top(., "Overall net benefit", 6, 7) %>% 
  add_header_above(c(" " = 2, "System" = 3)) %>% 
    footnote(general = "RCT: randomized controlled trial; GRADE: Grades of Recommendation, Assessment, Development, and Evaluation; ACC/AHA: American College of Cardiology/American Heart Association; USPSTF: U.S. Preventive Services Task Force",
    general_title = "",
    alphabet = c("Benefit ≥ Risk"),
    footnote_as_chunk = FALSE)
...
<font size = 4> Table `r table_n`. Residual gastric volumes at induction in randomized controlled trials of adult patients undergoing surgery according to fasting, liquid and volume. </font>
...
gum_rgv <- gum_rgv %>% 
  ...
  slice(-c(4,10)) %>% 
  mutate(rangel = as.numeric(str_split(range, " to ", simplify = TRUE)[,1]),
         ...
         med     = ifelse(refid == 4926, str_c(med, "^d^"), med),
         iqr     = ifelse(refid == 4926, str_c(iqr, "^d^"), iqr),
...
gum_rgv %>% 
  ...
  pack_sub(., "RCT", 1, 16) %>% 
  pack_top(., "Adult, Non-surgical", 17, 18) %>% 
  pack_sub(., "Crossover", 17, 18) %>% 
  pack_sub(., "Case Series", 19, 19) %>% 
...
```{r rgvGumMeta, include = TRUE, echo = FALSE, fig.width = 9.75, fig.height = 5.0, fig.align = "top"}
...
temp_rgv <- gum_rgv %>% 
  ...
  mutate(mean = ifelse(refid == 2364, mean * 70, mean),
...
rgv_meta <- metacont(n.e = n.y, n.c = n.x, 
         mean.e = mean.y,
         sd.e = sd.y,
         median.e = med.y, 
         q1.e = iqrl.y, 
...
<font size = 4> Figure `r figure_n`. Summary and individual study risk of bias appraisal for pooled randomized controlled trials examining residual gastric volume. </font>

<img src="assets/gum_rgv_rob_summary.svg" style="width:600px;" align="left" />

<img src="assets/gum_rgv_rob_traffic.svg" style="width:600px;" align="left" />

<br clear="all" />

```{r robFigure2_rgv, echo = FALSE, out.width = 700, out.height = 300, fig.align = "left", dev = "svg"}
figure_n <- fig_inc()
```
...
gum_ph %>% 
  ...
  mutate(n = ifelse(is.na(n), "NR", n)) %>% 
  kable(booktabs = T, align = c("llccccccc"), escape = FALSE,
  col.names = c("\\ \\ \\ \\ \\ \\ \\ Author", "Treatment", "N", "Time (hr)^a^", "\\ Measured", "Mean (SD)", "Median", "Range", "IQR")) %>%    
  kable_classic(full_width = TRUE, html_font = "Source Sans Pro", position = "left", "hover") %>% 
  kable_styling(bootstrap_options = c("striped", "hover","condensed")) %>% 
  row_spec(gum_row, color = "brown") %>% 
  add_header_above(c(" " = 3, " " = 1, "Time" = 1, " " = 4)) %>% 
...
```{r phGumMeta, include = TRUE, echo = FALSE, fig.width = 9.75, fig.height = 5.0, fig.align = "top"}
temp_ph <- readxl::read_xlsx("data/GumOutcomes_102820.xlsx", sheet = "pH", range = "A8:L39") %>% 
  janitor::clean_names()
...
ph_meta <- metacont(n.e = n.y, n.c = n.x, 
         mean.e = mean.y,
         sd.e = sd.y,
         median.e = med.y, 
         q1.e = iqrl.y, 
...
<font size = 4> Figure `r figure_n`. Summary and individual study risk of bias appraisals for pooled parallel randomized controlled trials (n = 4) examining residual gastric volume. </font>

<img src="assets/gum_ph_rob_summary.svg" style="width:600px;" align="left" />

<img src="assets/gum_ph_rob_traffic.svg" style="width:600px;" align="left" />

<br clear="all" />


```{r robFigure1_ph, echo = FALSE, out.width = 675, out.height = 300, fig.align = "left", dev = "svg"}
knitr::include_graphics("figures/ph_rob_summary.svg")
# figure_n <- fig_inc()
```

```{r robFigure2_ph, echo = FALSE, out.width = 700, out.height = 300, fig.align = "left", dev = "svg"}
## Individual Trials  
# <font size = 4> Figure `r figure_n`. Individual study risk of bias appraisals for included randomized controlled trials examining . </font>
knitr::include_graphics("figures/ph_rob_traffic.svg")
figure_n <- fig_inc()
```
...
temp_thirst <- readxl::read_xlsx("data/GumOutcomes_102820.xlsx", sheet = "thirst", range = "A7:H9") %>% 
  janitor::clean_names()

gum_thirst <- temp_thirst %>% 
  filter(!is.na(refid)) %>% 
  mutate(iqrl = as.numeric(str_split(iqr, " to ", simplify = TRUE)[,1]),
         iqru = as.numeric(str_split(iqr, " to ", simplify = TRUE)[,2]),
         study = str_replace(study, "\\(", ""),
         study = str_replace(study, "\\)", ""),
         tx = ifelse(str_detect(tx, "free"), "Gum sugarfree", tx),
...
gum_thirst %>% 
  select(study:iqr) %>%
  mutate(scale = "0➞10",
         scale = ifelse(is.na(study), NA, scale)) %>% 
  mutate(n = ifelse(is.na(n), "NR", n)) %>% 
  kbl(booktabs = T, align = c("llcccccc"), escape = FALSE,
  col.names = c("\\ \\ \\ \\ \\ \\ \\ Author", "Treatment", "N", "Time (hr)^a^", "Chewing", "Median", "IQR", "Scale^b^")) %>%    
  kable_classic(full_width = TRUE, html_font = "Source Sans Pro", position = "left", "hover") %>% 
  kable_styling(bootstrap_options = c("striped", "hover","condensed")) %>% 
  row_spec(gum_row, color = "brown") %>% 
  add_header_above(c(" " = 3, "Last Chewing" = 1, "Time" = 1, " " = 3)) %>% 
  column_spec(1, width = "8em") %>% 
  column_spec(2, width = "6em") %>% 
  column_spec(3, width = "3em") %>%
  column_spec(4, width = "3em") %>%
  ...
  pack_sub(., "RCT", 1, 2) %>% 
  footnote(general = "Med: median; IQR: interquartile range; RCT: randomized controlled trial; NR: not reported.", 
    general_title = "",
    alphabet = c("Before induction", "Numeric rating scale: 0 (without thirst) to 10 (intense thirst)"),
    footnote_as_chunk = FALSE)
```

<br>

<font size = 4> Figure `r figure_n`. Calculated mean difference in thirst scores in Garcia 2017---gum chewing versus fasting. </font>

```{r thirstGumMeta, include = TRUE, echo = FALSE, fig.width = 9.75, fig.height = 2.5, fig.align = "top"}

temp_thirst <- gum_thirst %>% 
  select(study, tx, n, med, iqrl, iqru)

thirst_meta.dat <- left_join(gum_thirst[c(1), ], gum_thirst[c(2), ], 
                          by = c("refid"))
  
thirst_meta <- metacont(n.e = n.y, n.c = n.x, 
         median.e = med.y, 
         q1.e = iqrl.y, 
         q3.e = iqru.y,
         median.c = med.x, 
...
forest(thirst_meta,
       ...
       #xlim = c(-20, 30),
       fs.xlab = 11,
       xlab = "Thirst (0-10 NRS)\n Favors Gum             Favors Fasting")

figure_n <- fig_inc()
```

<font size = 2> MD: mean difference; NRS: numeric rating scale. <br>Means calculated from medians and interquartile ranges. Authors reported standardized mean difference of 0.60 (p\< 0.001) obtained from p-value.</font>

<br>

## Safety

<font size = 4> Shanmugam et al. (2016) reviewed 9 chewing gum incidents reported to the WebAIRS database between 2005 and 2016. Four patients were considered at high risk for aspirating gum, but none did. In 3 patients, gum was identified in the recovery room and in 1 near the vocal cords.

The authors concluded, "We recommend that patients should be specifically advised to avoid gum chewing once fasting from clear fluids is commenced, and that a specific question regarding the presence of chewing gum should be added to all preoperative checklists." </font>
...
# add footnote by refid, variable
# mutate(ifelse(refid == refid_foo, variable = str_c(variable, foot), variable))
# mutate(ifelse(refid == 353, variable = str_c(surg, "$^a$"), variable))
...
study_char_table %>%
  select(-design) %>% 
  kable(booktabs = T, align = c("lcccccccclc"), 
      # format = "latex",
  ...
  pack_top(., "Pediatric, Surgical", 10, 10) %>% 
  pack_sub(., "RCT", 10, 10) %>% 
  footnote(general = "Gen: general; Reg: regional; Sed: sedation; Hosp: hospital; Amb: ambulatory",
           general_title = "",
           footnote_as_chunk = T) # %>% 
...
pt_char_table <- study_arm_gum.dat %>% 
  ...
  filter(row_number()==1) %>% 
  ungroup() %>% 
  mutate(N = ifelse(refid == 4926, 46, N),
...
fasting_table <- study_arm.dat %>% 
  ...
  select(refid, tx, arm_n, fastsolid, fastliquid, type_of_liquid, timeingest1, amtingest1, timeingest2, amtingest2, age, surg_nosurg, design)
...
fasting_table <- fasting_table %>%
  mutate(tx = factor(tx,
    levels = c("Fasting", "Water", "Gum Sugared", "Gum Sugarfree", "Gum Nicotine", "Gum (any)", "Lollipop"))) %>% 
...
fasting_table %>%
  select(-(type_of_liquid:amtingest2), -refid, -design, -year, -age, -surg_nosurg) %>% 
  add_column(temp = NA) %>% 
  select(study:fastliquid, temp, everything()) %>% 
  kbl(booktabs = T, align = c("llccccll"),
      col.names = c("\\ \\ \\ \\ \\ \\ \\ Author", "Comparator", "N", "Solids", "Liquids", " ",  "Chewing or Time", "Gum Chewing")) %>% 
