...
study_char_prot.dat %>%
  ...
  pack_sub(., "RCT", 1, 21) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 20, 20) %>%
  pack_sub(., "Retrospective Cohort", 21, 21) %>%
  pack_top(., "Adult, Nonsurgical", 22, 28) %>%
  pack_sub(., "RCT", 22, 22) %>%
  pack_sub(., "Crossover", 23, 28) %>%
...
## (updated 2021/02/14 16:48) no subgroup dat ----------------------------
# no subgroup data
...
mutate(nausea_bar = str_replace(nausea_bar, ">\\d+", ">&nbsp;"),
       nausea_bar = ifelse(grepl("0.00%", nausea_bar), NA, nausea_bar))
...
dichot_regurg$regurg_bar <- ifelse(grepl("Prot", dichot_regurg$arm_tx),
...
dichot_regurg %>% 
  ...
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein.",
           alphabet = c("At time of endocoscopy = 0",
           "Regurgitation after upper endoscopic intubation."),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
<font size = 4> Table `r table_n`. Occurrence of vomiting according to fasting, liquid and volume. </font>

```{r vomitTableOr, include = TRUE}
## (updated 2021/01/12 09:42) vomitTableOr -------------------------------
dichot_format <- function(x) (x/100)
...
mutate(vomit_bar = str_replace(vomit_bar, ">\\d+", ">&nbsp;"),
       vomit_bar = ifelse(grepl(" 0.00%", vomit_bar), NA, vomit_bar))
...
dichot_vomit %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6→2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Fasting")) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Vomiting" = 2, "OR (95% CI)"), line = TRUE) %>% 
  kable_styling(bootstrap_options = opt_boot, position = "left") %>% 
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>% 
  column_spec(1, width = "13em") %>%  
  column_spec(2, width = "3em") %>% 
  ...
  pack_sub(., "RCT", 1, 12) %>%
  pack_sub(., "Retrospective Cohort", 13, 14) %>%
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein.",
           alphabet = c("Relative to induction or start of endocoscopy."),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
<font size = 4> Figure `r figure_n`. Pooled incidence of vomiting --- randomized trials comparing protein-containing drinks with fasting in adult surgical patients. </font>

```{r vomitMeta, echo = FALSE, fig.width = 10, fig.height = 3, fig.align = "center"}
## (updated 2020/12/13 09:57) vomitMeta ----------------------------------
# data
nvra_meta.dat <- dichot_prot.dat %>% 
    mutate(
    time_outcome = ifelse(time_outcome == 0, "Preop", time_outcome),
    time_outcome = ifelse(suppressWarnings(!is.na(as.numeric(time_outcome))), str_c("Postop ", time_outcome, " hrs"), time_outcome)) %>% 
  select(refid, study, year, arm_tx, vomit_n, arm_n, design, time_outcome) %>% 
  filter(design == "rct", !is.na(vomit_n), arm_tx != "CHO") %>% 
  mutate(arm_tx = as.character(arm_tx)) %>% 
  select(!design) %>% 
  arrange(year)

nvra_meta.dat <- left_join(nvra_meta.dat[c(1, 3, 5, 7, 9, 11), ],
  nvra_meta.dat[c(2, 4, 6, 8, 10, 12), ],
  by = c("refid", "study", "year")) %>%
  filter(arm_tx.x == "Fasting") %>% 
  filter(study != "Pexe-Machado 2013") %>% 
  mutate(post_op = ifelse(time_outcome.y == "Preop", "Preop", "Postop")) %>% 
  select(-contains("time"))

nvra_meta <- metabin(vomit_n.y, arm_n.y, vomit_n.x, arm_n.x,
  studlab = study,
  data = nvra_meta.dat,
  sm = "OR",
  method = "Inverse",
...
<font size = 4> Table `r table_n`. Complications reported in surgical studies. </font>    
...
prot_compl.dat <- dichot_prot.dat %>% 
  ...
  mutate(compl_descript = firstup(compl_descript),
         compl_descript = ifelse(study == "Perrone 2011" & arm_tx == "Water", "Anesthetic or postoperative", compl_descript)) %>% 
 group_by(study) %>%
mutate(
  study = ifelse(row_number() == 1, study, NA),
  ...
  filter(!refid %in% c(6446, 247)) %>% 
  mutate(study = ifelse(refid == 165 & !is.na(study), "Marquini 2020", study)) %>% 
ungroup() %>% 
  select(study, arm_tx, arm_n, hr_gt6:hr_2, complicate_np, compl_descript)
...
prot_compl.dat %>% 
   kbl(booktabs = T, align =  c("llcccccl"), escape = FALSE,
      col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "Liquid", "N", "> 6", "6→2", "≤ 2", "N (%)", "Description")) %>%
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Complications" = 2)) %>% 
  column_spec(1, width = "10em") %>%  
...
```{r complicateTableNoReport, include = FALSE}
## (updated 2021/01/12 18:04) aspirateTableReport -------------------------
prot_compl.dat <- dichot_prot.dat %>% 
  select(refid:time_outcome, complicate_np, hypotension_np, satisfac_np, compl_descript) %>% 
  filter_at(vars(complicate_np, hypotension_np, satisfac_np, compl_descript), any_vars(!is.na(.))) %>% 
...
mutate(
  ...
  compl_descript = ifelse(row_number() == 1, compl_descript, NA)) %>% 
ungroup() %>% 
  select(refid, study, arm_tx, arm_n, hr_gt6:hr_2, complicate_np, compl_descript)

temp <- prot_compl.dat %>% 
  group_by(refid) %>% 
  fill(study) %>% 
  ungroup() %>% 
  mutate(compl_n = as.numeric(str_extract(complicate_np, "^\\d{1}"))) %>% 
  # exclude duplicate Marquini publications
  filter(!refid %in% c(6446, 247)) %>% 
  mutate(study = ifelse(refid == 165, "Marquini 2020", study)) %>% 
  select(study, arm_n, compl_n, arm_tx) %>% 
...
bp.dat <- contin.dat %>% 
  ...
  arrange(age, desc(surg_nosurg), design, year, study, arm_tx) %>% 
  # select(refid, study, arm_tx, arm_n, sbp_time_1:dbp_sd_2, sbp_median_1, sbp_median_2, dbp_median_1, dbp_median_2) %>% 
  mutate(sbp_mean_1 = ifelse(refid == 1463, sbp_median_1, sbp_mean_1),
         dbp_mean_1 = ifelse(refid == 1463, dbp_median_1, dbp_mean_1),
         sbp_mean_2 = ifelse(refid == 1463, sbp_median_2, sbp_mean_2),
         dbp_mean_2 = ifelse(refid == 1463, dbp_median_2, dbp_mean_2)) %>% 
  select(!c(dbp_time_1, dbp_time_2)) %>% 
  select(study, arm_tx, arm_n, sbp_time_1, sbp_mean_1, sbp_sd_1, dbp_mean_1, dbp_mean_1, dbp_sd_1, sbp_time_2, sbp_mean_2, sbp_sd_2, dbp_mean_2, dbp_sd_2) %>%
  mutate(sbp_sd_1 = as.character(round_0(sbp_sd_1)),
         sbp_sd_2 = round_0(sbp_sd_2),
         dbp_sd_1 = round_0(dbp_sd_1),
         dbp_sd_2 = round_0(dbp_sd_2),
         sbp_mean_1 = round_1(sbp_mean_1),
         sbp_mean_2 = round_1(sbp_mean_2),
         dbp_mean_1 = round_1(dbp_mean_1),
         dbp_mean_2 = round_1(dbp_mean_2),
         sbp_sd_1 = str_c("(", sbp_sd_1, "\\)"),
         sbp_sd_2 = str_c("(", sbp_sd_2, "\\)"),
         dbp_sd_1 = str_c("(", dbp_sd_1, "\\)"),
         dbp_sd_2 = str_c("(", dbp_sd_2, "\\)"),
         sbp_sd_1 = ifelse(sbp_sd_1 == "(NA\\)", NA, sbp_sd_1), 
         sbp_sd_2 = ifelse(sbp_sd_2 == "(NA\\)", NA, sbp_sd_2), 
         dbp_sd_1 = ifelse(dbp_sd_1 == "(NA\\)", NA, dbp_sd_1), 
         dbp_sd_2 = ifelse(dbp_sd_2 == "(NA\\)", NA, dbp_sd_2), 
...
cont_format <- function(x) (x/200)

bp.dat$sbp_mean_1 <- color_bar("burlywood", fun = cont_format)(bp.dat$sbp_mean_1)
bp.dat$sbp_mean_2 <- color_bar("burlywood", fun = cont_format)(bp.dat$sbp_mean_2)
bp.dat$dbp_mean_1 <- color_bar("darksalmon", fun = cont_format)(bp.dat$dbp_mean_1)
bp.dat$dbp_mean_2 <- color_bar("darksalmon", fun = cont_format)(bp.dat$dbp_mean_2)

bp.dat <- bp.dat %>% 
  mutate(dbp_mean_1 = str_replace(dbp_mean_1, "NA   ", ""),
         sbp_mean_2 = str_replace(sbp_mean_2, "NA   ", ""),
         dbp_mean_2 = str_replace(dbp_mean_2, "NA   ", ""),
         dbp_mean_2 = str_replace(dbp_mean_2, "padding-right: 2px;", "padding-right: 0px;"))
#dbp_mean_2 = ifelse(grepl("Oyama", study), str_replace(dbp_mean_2, "padding-right: 2px;", "padding-: 20px;"), dbp_mean_2))
...
bp.dat %>% 
  mutate(sbp_time_2 = ifelse(arm_n == 5, "2^a^", sbp_time_2)) %>% 
  add_row() %>% 
  # footnote for Singh 2015 time 
  ...
  kable_classic(position = "left", full_width = TRUE, html_font = opt_font, "hover") %>% 
  column_spec(1, width = "13em") %>%  
  column_spec(2, width = "6em") %>% 
  column_spec(3, width = "2em") %>% 
  column_spec(c(4,6,8,11,13), width = "3em") %>% 
  column_spec(c(5,7,9,10,12), width = "5em") %>% 
  row_spec(protein_row, color = "brown") %>% 
  pack_top(., "Adult, Surgical", 1, 8) %>% 
  pack_sub(., "RCT", 1, 4) %>%
  pack_rows("Difference detected at the end of surgery in SBP between water and fasting arms^b^", 5, 5, label_row_css = "padding-left:40%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
  pack_sub(., "Nonrandomized Studie of Interventions", 5, 8) %>%
  pack_rows("Differences not detected betweenarms for SBP or DBP", 7, 7, label_row_css = "padding-left:40%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
  pack_rows("Differences not detected between arms for SBP or DBP", 9, 9, label_row_css = "padding-left:40%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
  footnote(general = "RCT: randomized controlled trial; Hr: hour; SBP: systolic blood pressure; DBP: diastolic blood pressure; SD: standard deviation.",
           general_title = "", 
           alphabet = c("Estimated approximate time for end of surgery.",
                        "Sample size precludes conclusions."),
           footnote_as_chunk = FALSE)
...
```{r phTableUnreportedRemove, include = FALSE, eval = FALSE}
## pH
# Unreported in included studies.
# was Riley 2015 excluded
...
ph.dat %>% 
  ...
  pack_rows("MD 1.30 (95% CI, 0.17 to 2.43)", 3, 3, label_row_css = "padding-left:55%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
...
```{r dichotProtNonSurgRemove, include = TRUE, eval = FALSE}
...
<font size = 4> Table `r table_n`. Reported length of stay according to liquid or fasting.^a^ </font>
...
study_char_table %>%
  ...
  pack_sub(., "Nonrandomized Studies of Interventions", 20, 20) %>% 
  pack_sub(., "Retrospective Cohort", 21, 21) %>% 
  pack_top(., "Adult, Nonsurgical", 22, 22) %>% 
  pack_sub(., "RCT", 22, 22) %>%
  pack_sub(., "Crossover", 23, 28) %>%
  pack_top(., "Pediatric, Nonsurgical", 29, 29) %>% 
  pack_sub(., "RCT", 29, 29) %>% 
...
fasting_table %>%
  ...
  pack_sub(., "RCT", 1, 44) %>% 
  pack_sub(., "Nonrandomized Studies of Interventions", 45, 48) %>% 
  pack_sub(., "Retrospective Cohort", 49, 50) %>% 
  pack_top(., "Adult, Nonsurgical", 51, 66) %>% 
  pack_sub(., "RCT", 51, 52) %>%
  pack_sub(., "Crossover", 53, 66) %>%
  pack_top(., "Pediatric, Nonsurgical", 67, 69) %>% 
  pack_sub(., "RCT", 67, 69) %>%   
...
study_fund_table %>% 
  ...
  pack_top(., "Adult, Surgical", 1, 19) %>% 
  pack_sub(., "RCT", 1, 17) %>% 
  pack_sub(., "Nonrandomized Studies of Interventions", 18, 18) %>% 
  pack_sub(., "Retrospective Cohort", 19, 19) %>% 
  pack_top(., "Adult, Nonsurgical", 20, 20) %>% 
  pack_sub(., "RCT", 20, 20) %>%
  pack_sub(., "Crossover", 21, 26) %>%
  pack_top(., "Pediatric, Nonsurgical", 27, 27) %>% 
  pack_sub(., "RCT", 27, 27)
  #footnote(general = "g: grams; Osm: osmolality",
          # alphabet = c("Hours prior to surgery"),
          # alphabet_title = " ",
