...
```{r}
## (updated 2021/02/13 08:19) study_char_cho.dat --------------------------
study_char_cho.dat %>%
  ...
  add_header_above(c(" " = 1, "Adult" = 2, "Pediatric" = 2), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
...
dichot_hunger %>%
  ...
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Hunger" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>%
  row_spec(0, bold = TRUE) %>% 
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
  ...
  pack_top(., "Pediatric, Surgical", 13, 14) %>%
  pack_sub(., "RCT", 13, 14) %>%
  footnote(
...
## &ensp; Pooled (adult RCTs) <a id="fig1"></a>

<br>

<font size = 4> Figure `r figure_n`. Pooled results from randomized trials reporting rates of hunger comparing carbohydrate drinks with fasting in adult surgical patients. </font>
...
likert_cho_hung_kbl.dat %>%
  ...
  kbl(booktabs = T, align = c("llccccclll"), escape = FALSE,
      col.names = c(
        ...
        "Med (IQR)", "Range")) %>%
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Hunger Rating" = 5), line = TRUE, bold = TRUE) %>%
  row_spec(0, bold = TRUE) %>% 
  ...
  pack_sub(., "Nonrandomized Studies of Interventions", 65, 70) %>% 
    pack_rows("No detected differences", 69, 69, label_row_css = "padding-left:78%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
  pack_rows("Difference  p < 0.0001", 71, 71, label_row_css = "padding-left:78%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
...
<br>
+
## &ensp; Pooled (adult RCTs, ratings) <a id="fig2"></a>
...
```{r hungerAdultLikertMeta, echo = FALSE, fig.width = 11, fig.height = 5, fig.align = "center"}
...
```
Results from 4 studies were not included in the meta-analysis. Naguib 2001 and Mellis 2006 did not report sufficient data to estimate effects. Estimated standard errors from ranges reported in Yagmudrur 2011 were exceedingly small yielding an outlier effect. Reported IQR from Zhang 2019 carbohydrate arm did not allow variance estimation. 


<a id="fig3"></a>
<br><br>

<font size = 4> Figure `r figure_n`. Pooled hunger ratings (VAS or NRS) from randomized trials comparing carbohydrate drinks with clear liquids or water in adult surgical patients. </font>

```{r hungerAdultLikertMetaClear, echo = FALSE, fig.width = 11, fig.height = 4.5, fig.align = "center"}
hung_lik_meta.dat <- likert_cho_hung_kbl.dat %>%
  filter(age == "Adult", design == "rct") %>%
  # remove Naguib Melis lacking data to pool
  # Results from 4 studies were not included in the meta-analysis. dNaguib 2001 and Mellis 2006 did not report sufficient data to estimate effects. Estimated standard errors from ranges reported in Yagmudrur 2011 were exceedingly small yielding an outlier effect. Reported IQR from Zhang 2019 carbohydrate arm did not allow variance estimation.
...
# fasting
clear_meta.dat <- hung_lik_meta.dat %>%
  filter(arm_tx %in% c("Clear", "Water", "CHO", "CHO/Prot", "Placebo")) %>%
  group_by(refid) %>%
  filter(n() > 1) %>%
  #filter(row_number() < 3) %>%
  ungroup() %>%
...
```{r hungerAdultLikertFunnel, echo = FALSE, fig.width = 6, fig.height = 4, fig.align = "center", include = FALSE, eval = FALSE}
...
<font size = 4> Table `r table_n`. Rates of thirst according to fasting, liquid and volume. </font>
...
naguib <- tribble(
  ...
  39, 75, 24, 75,  "fasting",
  39, 75, 18, 75,  "water",
  39, 75, 41, 75,  "apple juice",
  39, 75, 38, 75,  "honey/cho")

naguib <- with(naguib, odds_ratio(event_e, n_e, event_c, n_c, arm)) %>% mutate(refid = 5803)
dichot_thirst$or_ci <- replace(dichot_thirst$or_ci, c(1:4), naguib$or_ci)
rm(naguib)
...
dichot_thirst %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  row_spec(0, bold = TRUE) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Thirst" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  kable_styling(bootstrap_options = opt_boot, position = "left") %>%
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>%
  column_spec(1, width = "13em") %>%
  column_spec(2, width = "3em") %>%
  ...
  pack_top(., "Pedatric, Surgical", 19, 20) %>% 
  pack_sub(., "RCT", 19, 20) %>%
...
## &ensp; Pooled (adult RCTs) <a id="fig4"></a>

<br>

<font size = 4> Figure `r figure_n`. Pooled rates of thirst --- randomized trials comparing carbohydrate drinks with fasting in adult surgical patients. </font>
...
Paule-Mandel estimator for between-study variance and Hartung-Knapp method for random effects. 
...
<font size = 4> Table `r table_n`. Incidence of nausea according to fasting, liquid and volume. </font>
...
dichot_nausea <- left_join(dichot_nausea, or_nausea, by = "refid") %>% 
  ...
  select(refid, arm, design, age, surg_nosurg, study, arm_n, arm_tx, starts_with("hr"), time_outcome, nausea_np, nausea_per, or_ci)
...
dichot_nausea %>% 
  select(!c(refid, arm, design, age, nausea_per, surg_nosurg)) %>% 
  select(study:nausea_np, nausea_bar, or_ci) %>% 
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  row_spec(0, bold = TRUE) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Nausea" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>% 
  kable_styling(bootstrap_options = opt_boot) %>% 
  ...
  column_spec(10, width = "5em") %>% 
  row_spec(cho_row, color = "brown") %>% 
  pack_top(., "Adult, Surgical", 1, 51) %>% 
  pack_sub(., "RCT", 1, 47) %>%
  pack_sub(., "Retrospective Cohort", 48, 49) %>%
  pack_sub(., "Before-After", 50, 51) %>%
  # pack_sub(., "Nonrandomized Studies of Interventions", 9, 10) %>%
  pack_top(., "Pedatric, Surgical", 52, 56) %>% 
  pack_sub(., "RCT", 52, 55) %>%
  pack_sub(., "Prospective Cohort", 56, 56) %>%
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein.",
           alphabet = c("When outcome assessed relative to surgery where 0 is prior to induction."),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
## &ensp; Pooled (adult RCTs) <a id="fig5"></a>
...
```{r nauseaMetaDichot, include = TRUE,  fig.width = 10, fig.height = 5.5, fig.align = "center"}
## (updated 2021/01/13 09:48)  --------------------------------------------
# data
# cho vs fasting
...
dichot_comfort %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Comfort" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>% 
  kable_styling(bootstrap_options = opt_boot) %>% 
  column_spec(1, width = "9em") %>%  
...
likert_cho_satis_kbl.dat %>%
  ...
  pack_rows("CHO vs Water:\\  \\ MD^f^ 2.00 (95% CI, 1.66-2.34) CHO vs Fasting:\\ MD^f^ 4.00 (95% CI, 3.71-4.29)", 4, 4, label_row_css = "padding-left:68%; border-bottom: 0px solid;", underline = FALSE, bold = FALSE, color = "gray", italic = TRUE) %>% 
...
## &ensp; Not Reported <a id="aspirate2"></a>
...
# dichot_regurg %>% 
#   filter(grepl("CHO", arm_tx) | arm_tx == "Fasting") %>% 
#   filter(design == "rct") %>% 
#   summarize(tot_n = sum(arm_n))
...
dichot_regurg %>% 
  ...
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Regurgitation" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
  kable_styling(bootstrap_options = opt_boot, position = "left") %>% 
  kable_classic(full_width = FALSE, html_font = opt_font, position = "left", "hover") %>% 
  column_spec(1, width = "13em") %>%  
...
<font size = 4> Table `r table_n`. Rates of vomiting according to fasting, liquid and volume. </font>
...
dichot_vomit %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Vomiting" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>% 
  kable_styling(bootstrap_options = opt_boot) %>% 
  column_spec(1, width = "9em") %>%  
  column_spec(2, width = "3em") %>% 
  column_spec(3, width = "4em") %>% 
  column_spec(c(4), width = "2em") %>% 
  column_spec(c(5), width = "4em") %>% 
  column_spec(c(6), width = "2em") %>% 
  column_spec(5, width = "3em") %>% 
  column_spec(7, width = "3em") %>% 
  column_spec(8, width = "4em") %>% 
  column_spec(9, width = "5em") %>% 
...
## &ensp; Pooled (adult RCTs) <a id="fig7"></a>

<br>

<font size = 4> Figure `r figure_n`. Pooled rates of vomiting --- randomized trials comparing carbohydrate drinks with fasting in adult surgical patients. </font>

```{r vomitMetaDichot, include = TRUE,  fig.width = 10, fig.height = 5.5, fig.align = "center"}
## (updated 2021/01/13 09:48)  --------------------------------------------
# data
# cho vs fasting
...
<font size = 4> Figure `r figure_n`. Pooled rates of vomiting --- randomized trials comparing carbohydrate drinks with water or placebo in adult surgical patients. </font>
...
forest(vomit_meta,
  ...
  subgroup = FALSE,
  pooled.events = TRUE,
  xlab = "Favors CHO       Favors Water/Clear")
...
<font size = 4> Table `r table_n`. Reported complications according to fasting, liquid and volume in adults (unreported in pediatric studies). </font>
...
mutate(complicate_bar = str_replace(complicate_bar, ">0", ">"),
       ...
       study = ifelse(study == "Song 2016", cell_spec(study, color = "black"), study),
       or_ci = ifelse(refid == 147 & arm_n == 275,  cell_spec(or_ci, color = "black"), or_ci))
...
dichot_complicate %>% 
  ...
  kbl(booktabs = T, align =  c("lclccccrlc"), escape = FALSE,
    col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "\\ \\ \\ N", "Liquid", "> 6", "6 → 2", "≤ 2", "(hr)", "N (%)", " ", "CHO vs. Comparator")) %>% 
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "Complications" = 2, "OR (95% CI)"), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>% 
  kable_styling(bootstrap_options = opt_boot) %>% 
  column_spec(1, width = "9em") %>%  
  ...
  column_spec(10, width = "5em") %>% 
  row_spec(cho_row, color = "brown") %>% 
  pack_top(., "Adult, Surgical", 1, 74) %>% 
  pack_sub(., "RCT", 1, 64) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 65, 68) %>%
  pack_sub(., "Prospective Cohort", 69, 69) %>%
  pack_sub(., "Retrospective Cohort", 70, 72) %>%
  pack_sub(., "Case-Control", 73, 74) %>%
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein.",
           alphabet = c("When outcome assessed relative to surgery where 0 is prior to induction."),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
<font size = 4> Figure `r figure_n`. Pooled complications rates --- randomized trials comparing carbohydrate drinks with fasting in adult surgical patients. </font>

```{r complicateMetaDichotDat, include = TRUE}
## (updated 2021/01/13 09:48)  --------------------------------------------
complicate_meta.dat <- dichot_cho.dat %>% 
  filter(!is.na(complicate_np)) %>%
  # Marquini b,c; nrsi Vigano 2012, Adamova 2017; cc Weledji 2017, coh Talutis 2020
  filter(!refid %in% c(6446, 247, 3367, 6529, 6530, 147)) %>% 
...
<font size = 4> Figure `r figure_n`. Pooled complications rates --- randomized trials comparing carbohydrate drinks with water, placebo, or clear liquids in adult surgical patients. </font>
...
rgv_surg <- rgv.dat %>%
  ...
  select(refid,study, arm_tx, arm_n, hr_gt6, hr_26, hr_2, rgv_time_1, rgv_med, rgv_mean, rgv_bar) %>%
  mutate(
...
rgv_surg %>%
  ...
  add_header_above(c(" " = 3, "Volume (mL) Before Induction (hr)" = 3, "Time^a^" = 1, "RGV (mL)" = 3), line = TRUE, bold = TRUE) %>%
  row_spec(0, bold = TRUE) %>% 
  kable_classic(full_width = TRUE, html_font = opt_font, position = "left", "hover") %>%
  kable_styling(bootstrap_options = opt_boot) %>%
  row_spec(cho_row, color = "brown") %>%
  ...
  pack_sub(., "RCT", 1, 33) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 34, 37) %>%
...
rgv_meta.dat <- contin.dat %>%
  ...
  filter(refid != 69) %>%  # exclude Karimian
...
<font size = 4> Figure `r figure_n`. Pooled residual gastric volume from randomized trials comparing carbohydrate drinks with water or clear liquids in adult surgical patients. </font>

```{r rgvChoMetaClear, include = TRUE, include = TRUE,  fig.width = 10, fig.height = 3.5, fig.align = "center"}
## (updated 2021/01/22 15:19) rgvChoMeta ----------------------------------
rgv_meta.dat <- contin.dat %>%
  arrange(design, year, study) %>% 
  filter(refid %in% cho_refids$refid, surg_nosurg == "surgical") %>%
  ...
  ungroup() %>%
  select(refid, study, year, design, arm_n, arm_tx, rgv_mean_1, rgv_sd_1, rgv_median_1:rgv_iqr_up_1) %>% 
  filter(!refid %in% c(5803, 3823, 3282)) %>% 
  # Naugib
...
rgv_meta.dat <- left_join(rgv_meta.dat[c(1,3,5,7,9), ], rgv_meta.dat[c(2,4,6,8,10), ], 
                          by = c("refid", "study", "year"))
...
<font size = 4> Table `r table_n`. Reported length of stay according to liquid or fasting. </font>
...
los.dat %>% 
   ...
   kbl(booktabs = T, align =  c("llcccc"), escape = FALSE,
      col.names = c("\\ \\ \\ \\ \\ \\ \\ Study", "Liquid", "N", "Mean (SD)", "Median (IQR)", "Range")) %>%
  add_header_above(c(" " = 3, "Length of Stay" = 3), line = TRUE, bold = TRUE) %>% 
  row_spec(0, bold = TRUE) %>% 
  ...
  row_spec(cho_row, color = "brown") %>% 
  pack_top(., "Adult, Surgical", 1, 95) %>% 
  pack_sub(., "RCT", 1, 79) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 80, 85) %>%
  pack_sub(., "Prospective Cohort", 86, 86) %>%
  pack_sub(., "Retrospective Cohort", 87, 91) %>%
  pack_sub(., "Case Control", 92, 93) %>%
  pack_sub(., "Before-After", 94, 95) %>%
  pack_top(., "Pediatric, Surgical", 96, 99) %>% 
  pack_sub(., "RCT", 96, 99) %>%
  footnote(general = "RCT: randomized controlled trial.",
           # alphabet = c("Only Pexe-Machado 2013 reported a difference between arms in length of stay (p = 0.04)."),
           general_title = "", 
           footnote_as_chunk = FALSE)
