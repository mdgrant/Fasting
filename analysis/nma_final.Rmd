...
rgv.dat <- contin_nma.dat %>%
  ...
  mutate(
    arm_tx = fct_collapse(arm_tx, Clear = c("Water", "Clear")),
...
trts <- c("CHO/Simp", "CHO/Comp", "CHO/Comp/Prot", "Clear", "Fasting")
...
as_tibble(df$table) %>%
  mutate(
    across(Fasting:Clear, ~ str_replace(., " \\(", "<br/>\\(")),
    across(Fasting:Clear, ~ if_else(.x %in% c("Fasting", "CHO/Comp", "CHO/Comp/Prot", "CHO/Simp", "Clear"),
...
order_dat <- tibble(Treatment = c("CHO/Simp", "CHO/Comp/Prot", "Clear", "Fasting"))
