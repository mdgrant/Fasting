...
# two_hours(likert_prot_hung_kbl.dat$refid)
...
kbl(
  ...
  result_pack("Volume (mL)                      2 hr after liquids", 9, padding = 45, italic = FALSE, color = "black", bold = TRUE) %>% 
...
dichot_comfort %>% 
  ...
  pack_top(., "Adult, Surgical", 1, 2) %>% 
  pack_sub(., "RCT", 1, 2) %>%
  footnote(general = "RCT: randomized controlled trial; OR: odds ratio; CHO: carbohydrate; Prot: protein; Comp: complex.",
           # alphabet = c(""),
           general_title = "",  
           footnote_as_chunk = FALSE)
...
likert_prot_comfort.dat <- likert_prot.dat %>%
...
likert_prot_comfort_kbl.dat <- likert_prot_comfort.dat %>%
...
# two_hours(likert_prot_comfort.dat$refid); verified only 2 hour

prot_row <- likert_prot_comfort_kbl.dat %>% 
  mutate(line = row_number()) %>% 
  filter(grepl("CHO", arm_tx)) %>% 
  pull(line)

likert_prot_comfort_kbl.dat %>% 
  select(-pre_post_op) %>% 
...
dichot_prot.dat %>%
  ...
  pack_top(., "Adult", 1, 14) %>%
  pack_sub(., "RCT", 1, 11) %>%
  pack_sub(., "Nonrandomized Studies of Interventions", 12, 14) %>%
  footnote(
...
totals <- dichot_prot.dat %>%
  ...
  filter(age == "Adult") %>%
  group_by(design) %>% 
  select(study, design, Fasting, Water, "CHO/Comp", "CHO/Comp/Prot") %>%
  adorn_totals(c("col", "row"), fill = "-", na.rm = TRUE) %>% 
  slice_tail(n = 1)
...
dichot_prot.dat %>%
  ...
  column_spec(6, width = "5em", bold = TRUE) %>%
...
```{r vomitPostopAdults, include = FALSE, eval = FALSE}
### Rates, Adults (postop) 
<br/>
